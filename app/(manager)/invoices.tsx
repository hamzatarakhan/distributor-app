...
import { useInvoices, useReps } from '@/src/hooks/data';
...
// Same shape as the rep's Invoices tab, plus who collected it (repName) and a filter to narrow
// the whole team's invoices down to one rep — a manager checking collections cares who to chase,
// a rep already knows every invoice here is theirs.
export default function ManagerInvoices() {
  ...
  const items = (data?.items ?? []).filter((i) => repFilter === 'all' || String(i.repId) === repFilter);
        ...
        {items.map((inv) => {
          ...
          return (
            <ListRow
              key={inv.id}
              title={inv.number}
              subtitle={`${inv.customerName} · ${inv.repName ?? '—'}`}
              right={
